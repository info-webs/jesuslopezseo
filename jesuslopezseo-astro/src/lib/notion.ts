...
const NOTION_DATABASE_ID =
  ...
  '819b3c45-d080-497b-915c-72ec68ccf633';
...
export async function getPosts(options?: {
  ...
  if (options?.category) {
    filter.and.push({ property: 'Category', select: { equals: options.category } });
  }
  const body: any = {
    filter,
...
export async function getPageContent(pageId: string, wpSlug?: string, originalUrl?: string): Promise<string> {
  ...
  if (res.ok) {
    ...
    if (html && html.replace(/<[^>]+>/g, '').trim().length > 200) {
      return html;
    }
  }
...
function blocksToHtml(blocks: any[]): string {
  ...
  for (const block of blocks) {
    ...
    switch (type) {
      case 'heading_1': html += `<h2>${richTextToHtml(content.rich_text)}</h2>`; break;
      case 'heading_2': html += `<h3>${richTextToHtml(content.rich_text)}</h3>`; break;
      case 'heading_3': html += `<h4>${richTextToHtml(content.rich_text)}</h4>`; break;
      case 'paragraph': { const text = richTextToHtml(content.rich_text); if (text) html += `<p>${text}</p>`; break; }
      case 'bulleted_list_item': if (listType !== 'ul') { flushList(); listType = 'ul'; } listBuffer.push(`<li>${richTextToHtml(content.rich_text)}</li>`); break;
      case 'numbered_list_item': if (listType !== 'ol') { flushList(); listType = 'ol'; } listBuffer.push(`<li>${richTextToHtml(content.rich_text)}</li>`); break;
      case 'quote': html += `<blockquote>${richTextToHtml(content.rich_text)}</blockquote>`; break;
      case 'code': html += `<pre><code>${richTextToHtml(content.rich_text)}</code></pre>`; break;
      ...
      case 'image': {
        ...
        if (imgUrl) html += `<figure><img src="${imgUrl}" alt="${caption}" loading="lazy"/>${caption ? `<figcaption>${caption}</figcaption>` : ''}</figure>`;
        break;
      }
      default: break;
    }
